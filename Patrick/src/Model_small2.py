...
def create_model(p, species_names, mesh_path, mesh_scale, plot_only_run):
    ...
    with mdl: # reaction diffusion environments
        ...
        nuc_mem = stmodel.VolumeSystem(name="nucleus_surface")
        ...
        # nucleus membrane
        with nuc_mem:
            stmodel.Diffusion(species_dict["ERKp"], rate = p["DC"])
    ...
    #rng = strng.RNG("mt19937", 512, 2903) # seed not working??
    rng = strng.RNG("mt19937", 512, np.random.randint(1,10**6)) # (type of RNG (mt: mersenne twister), number of pregenerated random numbers, seed value to initialize the RNG)
